            ...
            {events.map((e, i) => (
              <div key={i} style={styles.rowLine}>
                <div style={styles.left}>
                  {e.team === "blue" &&
                    `#${e.number} ${e.type === "goal" ? "🔵" : e.type==="miss"?"❌": e.type==="out"?"⛔":"↩"}`}
                </div>
                <div style={styles.center}>{e.score}</div>
                <div style={styles.right}>
                  {e.team === "red" &&
                    `${e.type === "goal" ? "🔴" : e.type==="miss"?"❌": e.type==="out"?"⛔":"↩"} #${e.number}`}
                </div>
              </div>
            ))}
            <div ref={bottomRef} />
...
const styles = {
  ...
  left: { textAlign: "right", color: "#60a5fa", whiteSpace: "nowrap" },
  center: { textAlign: "center", fontWeight: "bold" },
  right: { textAlign: "left", color: "#f87171", whiteSpace: "nowrap" },
